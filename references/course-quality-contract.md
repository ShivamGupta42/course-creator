...
- Every concept-first module must use learner-friendly sections: `Picture the Idea`, `Figure It Out From Scratch`, `How to Reason About This`, `Core Ideas`, `Real-World Anchor`, `Try a Small Case`, `Common Trap`, `Check Your Understanding`, `Practice in Levels`, and `Make It Yours`. A problem-first module uses the merged 10-section skeleton from SKILL.md ("Section budget and merge rule"): problem-first sections REPLACE overlapping standard sections, never stack on top of them, and the module has at most 10 `h2` sections.
- The six `Figure It Out From Scratch` cards (concept-first only) must each be scannable: a bold one-clause lead (`p.card-lead`) plus a short bullet list (`ul.card-points`), the whole card under ~25 words with each bullet 5-16 prose words. Dense 60-80 word paragraphs inside these cards are a failure, and so are 2-4 word telegrams ("All need provenance"). The `Explain It to a Friend` teach-back stays a short prose paragraph.
- Explanation shape is gated, not aspirational: every general claim carries a concrete instance within two sentences; every reported number shows its inputs and arithmetic in the same section; the module addresses the learner as "you" (≥10 per module); ≥5 causal connectives per module; Flesch-Kincaid grade ≤ 11 for lesson prose after stripping `<code>`; `data-learner-need`, `data-example`, and `data-campus` are each ≥40 words; at most 2 `data-move` tags; no repeated reasoning-cycle litany across modules.
...
- learner-facing prose breaks the explanation-shape gates: FK grade > 11, <5 causal connectives, <10 "you/your", a 40-word floor hook under length, a reported number with no inputs in its section, >2 `data-move` tags, >10 `h2` sections, a worksheet label prefix (`Wrong belief:`, `Why it tempts:`, `Diagnostic case:`, `Repair:`) in visible prose, or a problem-first module missing `data-prompt` divs or revealing `data-problem-feedback` before the learner answers
